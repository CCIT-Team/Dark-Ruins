...
public class PlayerMovement_KSM : MonoBehaviour
{
    ...
    private List<KeyCode> _currentHeldKeys = new List<KeyCode>();

    void OnEnable()
    {
        InputManager_KSM.Instance.OnKeysHeld += HandleKeysHeld;
    }

    void OnDisable()
    {
        if (InputManager_KSM.Instance != null)
        {
            InputManager_KSM.Instance.OnKeysHeld -= HandleKeysHeld;
        }
    }
