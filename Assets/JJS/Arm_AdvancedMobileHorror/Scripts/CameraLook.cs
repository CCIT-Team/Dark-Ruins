...
namespace AdvancedHorrorFPS
{
    public class CameraLook : MonoBehaviour
    {
        ...
        private UnityEngine.InputSystem.InputAction mouseDeltaAction;
        ...
        void LateUpdate()
        {
            ...
            {
                h += Touchpad.Instance.HorizontalValue * horizontalRotationSpeed * 10 * Time.deltaTime * PlayerPrefs.GetFloat("MouseSensivity", 1);
                v -= Touchpad.Instance.VerticalValue * verticalRotationSpeed * 10 * Time.deltaTime * PlayerPrefs.GetFloat("MouseSensivity", 1);
            }
            else
            {
                Vector2 mouseDelta = mouseDeltaAction.ReadValue<Vector2>();
                h += mouseDelta.x * horizontalRotationSpeed * 0.5f * PlayerPrefs.GetFloat("MouseSensivity", 1);
                v -= mouseDelta.y * verticalRotationSpeed * 0.5f * PlayerPrefs.GetFloat("MouseSensivity", 1);
