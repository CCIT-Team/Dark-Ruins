...
namespace AdvancedHorrorFPS
{
    public class InventoryManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            leftAction = new InputAction(type: InputActionType.Button, binding: "<Keyboard>/a");
            leftAction.Enable();

            rightAction = new InputAction(type: InputActionType.Button, binding: "<Keyboard>/d");
            rightAction.Enable();
        }
        ...
        private void Update()
        {
            if (SlotContainer.activeSelf && CurrentItemIndex < ItemsInInventory.Count)
            {
                ...
                if (AdvancedGameManager.Instance.controllerType == ControllerType.PcAndConsole)
                {
                    ...
                    else if (rightAction.WasReleasedThisFrame())
                    {
                        Go_Right();
                    }
                }
