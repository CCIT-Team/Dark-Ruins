...
namespace AdvancedHorrorFPS
{
    public class ItemScript : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                interactAction.AddBinding("<Touchscreen>/press");
                interactAction.Enable();
            }
        }
        ...
        private void Update()
        {
            if (interactionType == InteractionType.PressAndHold)
            {
                if (HeroPlayerScript.Instance != null && HeroPlayerScript.Instance.currentLookingItem != null && HeroPlayerScript.Instance.currentLookingItem == this)
                {
                    ...
                    else if (mouseAction.WasReleasedThisFrame())
                    {
                        imageToFill.fillAmount = 1;
                        imageToFill.gameObject.SetActive(false);
                    }
            ...
            else if (interactionType == InteractionType.Hide)
            {
                ...
                if (interactAction.WasPressedThisFrame() && HeroPlayerScript.Instance.hidingPlace == GetComponent<HideScript>() && HeroPlayerScript.Instance.isHiding)
                {
                    Interact();
                }
            }
            ...
            {
                // Hiding currently. Let's get out:
                if (interactAction.WasPressedThisFrame() && GameCanvas.Instance.CurrentNote == GetComponent<NoteScript>() && GetComponent<NoteScript>().isReading)
                {
                    Interact();
                }
            }
